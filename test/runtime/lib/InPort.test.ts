...
describe("InPort", function () {
  ...
  it("should handle simple communication with a single publisher", async function () {
    ...
    assert.equal(pub.requested, 2);
    assert.equal(port.requested(), 2);

    assert.deepEqual(compSub.elements, []);
    pub.next(1);
    pub.next(7);
    await eventually(() => assert.deepEqual(compSub.elements, [1, 7]));
  ...
  it("should ensure message processing from publishers is asynchronous", async function () {
    const compSub = new MockSubscriber<number>();
    const port = new InPort<number>("test", compSub);
    const pub = new MockPublisher<number>();
    pub.subscribe(port.newSubscriber());
    ...
    pub.next(2);
    assert.deepEqual(compSub.elements, []);
    await eventually(() => assert.deepEqual(compSub.elements, [1, 2]));
  ...
  it("should handle demand correctly with multiple publishers", async function () {
    ...
    pubs[0].next(16);
    pubs[1].next(32);
    await eventually(() => assert.deepEqual(compSub.elements, [16, 32]));
    ...
    pubs[0].next(16);
    pubs[1].next(32);
    await eventually(() => assert.deepEqual(compSub.elements, [16]));
  ...
  it("should handle completions correctly with multiple publishers", async function () {
    ...
    pubs[1].complete();
    await eventually(() => assert.equal(port.subscriptionCount(), 2));
    assert.equal(compSub.status, "active");
    ...
    pubs[2].complete();
    await eventually(() => assert.equal(port.subscriptionCount(), 1));
    assert.equal(compSub.status, "active");

    pubs[0].complete();
    await eventually(() => assert.equal(port.subscriptionCount(), 0));
